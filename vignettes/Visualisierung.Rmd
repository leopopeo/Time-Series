...
innovation_sim_AR_1 <- ts_predict(AR_1, 1, all = T)
innovation_sim_AR_2 <- ts_predict(AR_2, 1, all = T)
innovation_sim_MA_1 <- ts_predict(MA_1, 1, all = T)
innovation_sim_MA_2 <- ts_predict(MA_2, 1, all = T)
innovation_sim_ARMA_1 <- ts_predict(ARMA_1, 1, all = T)
```

```{r, fig.width = 7, fig.asp = .5}
plot_inno <- function(real_series, predicted_series, ...){
  tbl <- tibble(series=c(real_series, NA), inno_obj = predicted_series, Lag=1:101)
...
raw <- data_hourly$temp[2000:2100]-mean(data_hourly$temp[2000:2100])
raw_full <- data_hourly$temp[2000:2130]-mean(data_hourly$temp[2000:2130])
