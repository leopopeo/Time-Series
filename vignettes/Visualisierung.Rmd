...
itsmr_prediction <- forecast(AR_1, #Time series data
                             M = NULL, #Data model
                             a = arma(AR_1, p=15, q=15), #ARMA model
                             h = 10, #Steps ahead
...
#Achtung!! Sehr lange Rechenzeit für länge > 1000
# periodo <- perio(data$temp[1:100000])
# length(periodo)
# plot_periodogram(periodo)
# periodo

plot_DL_1 <- function(series, dl, ...){
  tbl <- tibble(Series=series, DL_obj = dl, Lag=1:300)
...
raw <- data[[2]][2701:2970]-mean(data[[2]][2701:2970])
raw_full <- data[[2]][2701:3000]-mean(data[[2]][2701:3000])
predic <- DL_prediction(raw,len=30,all=T)
plot_DL_1(raw_full,predic,"Test")
#plot_timeseries(predic,DL_prediction(predic,len=30))
#plot_timeseries(predic,ts_predict(predic,steps=30))
```
