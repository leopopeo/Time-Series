...
```{r, fig.width = 7, fig.asp = .5}
plot_DL(AR_1, DL1, "AR(1)")
plot_DL(AR_2, DL2, "AR(2)")
plot_DL(MA_1, DL3, "MA(1)")
plot_DL(MA_2, DL4, "MA(2)")
plot_DL(ARMA_1, DL5, "ARMA(1)")
```
...
ierfür erstellen wir eine Prediciton-Funktion, welche uns für die vorherigen $n$ Werte den $n+1$ Wert vorhersagt:
...
innovation_prediction <- function(X){
   ...
   for(i in 10:length(X)){
     inno_help <- X[1:i]
     inno_prediction[i] <- ts_predict(inno_help,1)
   }
   inno_prediction
}
```
...
```{r, fig.width = 7, fig.asp = .5}
plot_inno <- function(real_series, predicted_series, ...){
  tbl <- tibble(series=real_series, inno_obj = predicted_series, Lag=1:100)
  
   ggplot(tbl, aes(x=Lag)) + 
    geom_line(aes(y = series), color = "blue", size=0.4,na.rm=T) + 
    geom_line(aes(y = inno_obj), color= "red", size=0.4,na.rm=T) +
    ggtitle(...)
}

plot_inno(AR_1, innovation_sim_AR_1, "AR")
plot_inno(AR_2, innovation_sim_AR_2, "AR")
plot_inno(MA_1, innovation_sim_MA_1, "MA")
plot_inno(MA_2, innovation_sim_MA_2, "MA")
plot_inno(ARMA_1, innovation_sim_ARMA_1, "ARMA")
```
...
```{r}
itsmr_prediction <- forecast(AR_1, #Time series data
                             M = NULL, #Data model
                             a = arma(AR_1, p=10, q=0), #ARMA model
                             h = 10, #Steps ahead
                             opt = 0, #Display option (0 silent, 1 tabulate, 2 plot and tabulate)
                             alpha = 1)$pred #Level of significance
innovation_sim_1 <- ts_predict(AR_1, 10)
```

```{r , fig.width = 7, fig.asp = .5}

plot_timeseries(AR_1, itsmr_prediction)
plot_timeseries(AR_1, innovation_sim_1)
```

Wir können zunächst festhalten, dass die "forecast"-Funktion dem Nutzer mehr Parameter zur Verfügung stellt. Mit einer geeigneten Wahl von q und p, welche allerdings vom hier vorliegenden Prozess abweicht, gelingt eine nahezu identische Vorhersage der nächsten 10 Datenpunkte.
...
raw <- data[[2]][2000:2100]-mean(data[[2]][2000:2100])
raw_full <- data[[2]][2000:2130]-mean(data[[2]][2000:2130])
