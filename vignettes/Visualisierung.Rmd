...
data_mon <- as_tibble(read.delim("C:/Users/Johannes Schnebel/Documents/Studium/VWL/R Kurs/Time-Series/vignettes/Luftdaten_monat_18810101_20201231_05906.txt",
                                 header = T,
                                 sep = ";",
                                 dec = "."))
...
data_mon %>%
  ...
  filter((abs(temp) < 50) & (20140101 <= date) ) %>%
...
periodo <- perio(data_mon$temp)

plot_periodogram(periodo, logscale = T)
```
An diesem Plott sehen den groben Tagestemperatur Trend. Die X-Achse ist hier in Stunden und in ungefähr 24 Stunden sehen wir ein Tal (Nacht) und ein Peak (Tag) in den Temperaturdaten. 
```{r, fig.width = 7, fig.asp = .5}
periodo <- perio(data_hourly$temp[3700:3800])

plot_periodogram(periodo, logscale = T)
```
Das Periodogramm hätten wir uns eigentlich ein wenig anders vorgestellt. Da der betrachtete Zeitraum eine relativ deutliche wiederkehrende Periode aufweißt hätten wir im Periodogramm einen Peak abseits der Null erwartet. Dies ist leider nicht der Fall.
