...
```
Auch hier ist der $MSE$ beim $MA(1)$-Prozess mit $2.00e-3$ am geringsten. Obwohl er optisch die Peaks nicht so gut erfasst wie beim $AR(1)$-Prozess. Die Vorhersage ist insbesondere immer flacher und nicht so Spitz wie in unserer tatsächlichen Zeitreihe, was unter anderem damit zusammenhängen kann, dass die ACF Funktion selbst starke Peaks erzeugt und der DLA nicht mit dem Rauschen klarkommt, was erklären würde, weswegen er nicht so stark ausschlägt, wie der Innovation Algorithmus. Der deutlich größere $MSE$ beim $ARMA(1,1)$-Prozess wird vermutlich hauptsächlich durch den großen Vorhersagefehler bei ca. *Lag = 40* verursacht. 
...
```
Zunächst können wir festhalten, dass die Ein-Schritt-Prognose mit dem Innovation-Algorithmus für alle Prozesse einen größeren $MSE$ liefert als mit Durbin-Levinson.
Bei den $MA$-Prozessen ist der $MSE$ geringer als bei den Prozessen mit $AR$-Anteil. Dies entspricht auch nach Brockwell unseren Erwartungen. Die Ausschläge bei den Extremwerden sind bei der Vorhersage ebenfalls nicht so stark wie bei der tatsächlichen Zeitreihe. Dies liegt daran, dass zur Prognose ein gewichtetes Mittel der Vergangenen Differenzen $\mathbf{X_n} - \mathbf{\hat X_n}$ herangezogen wird, was dazu führt, dass die Prognose tendenziell immer zum Mittelwert strebt. Dieses Verhalten der Vorhersage mit Innovation, Bewegungen zu antizipieren, aber zu dämpfen, sieht man besonders gut an folgendem Beispiel eines vollständig deterministischen periodischen Prozesses:
...
```
Zur Übersicht stellen wir hier noch einmal für den $AR(1)$-, $MA(1)$- und  $ARMA(1,1)$-Prozess jeweils die Vorhersage mit dem Durbin-Levinson- und dem Innovation-Algorithmus sowie die jeweilgen $MSE$s gegenüber.
...
Zunächst einmal betrachten wir das Ergebnis einer Ein-Schritt-Prognose der forecast-Funktion mit unserer ts_predict funktion für den $MA(1)$-Prozess.
Der $MSE = 9.73e-3$ ist bei der Prognose mit *forecast* nicht viel geringer als mit unserer Funktion *ts_predict* ($MSE = 10.8e-3$).
```{r, fig.width = 7, fig.asp = .5}
...
plot_mult(AR_1, innovation_sim_AR_1, itsmr_onestep_AR, "ts_predict red, forecast green.")
```

```{r , fig.width = 7, fig.asp = .5}
MSE(MA_1[-(1:11)], innovation_sim_MA_1[-(1:11)])
MSE(MA_1[-(1:11)], itsmr_onestep_MA[-(1:11)])
plot_mult(MA_1, innovation_sim_MA_1, itsmr_onestep_MA, "ts_predict red, forecast green.")
```

```{r , fig.width = 7, fig.asp = .5}
MSE(ARMA_1[-(1:11)], innovation_sim_ARMA_1[-(1:11)])
MSE(ARMA_1[-(1:11)], itsmr_onestep_ARMA[-(1:11)])
plot_mult(ARMA_1, innovation_sim_ARMA_1, itsmr_onestep_ARMA, "ts_predict red, forecast green.")
```
Der $MSE$ der Ein-Schritt-Prognose ist bei allen drei Prozessen für *forecast* und *ts_predict* nahe beieinander. Für den ARMA-Prozess ist unser Algorithmus nach $MSE$ besser.
...
```
Hier haben wir leider nicht das gewünsche Ergbenis erhalten. In der Dokumentation der *forecast*-Funktion steht zwar, das die Funktion ebenfalls den gleichen Innovatoin Algorithmus als Grundlage für die Berechnung benutzt, es ist jedoch trotzdem keine gute Übereinstimmung zwischen unserer Vorhersage und der *forecast*-Vorhersage zu erkennen. Die Spezifizierung des ARMA-Modells in der *forecast*-Funktion weicht hier vom generierten Prozess ab, auch für andere Werte *p*  und  *q*  in *forecast* ergibt sich kein besseres Ergebnis.

## Reale Daten

Da Temperaturdaten eine starke saisonale Komponente haben eignen sich diese gut für eine Analyse mit einem *ARMA*-Modell. Wir analysieren im Folgenden Temperaturdaten des DWD für Mannheim, einmal auf stündlicher und einmal auf monatlicher Basis.
...
Im Folgenden sehen wir zunächst für die stündlichen Werte neben der Zeitreihe (blau) je eine Vorhersage für *t=101* bis *t=131* mit dem Durbin-Levinson Algorithmus und dem Innovation-Algorithmus (rot).
```{r, fig.width = 7, fig.asp = .5}
...
```
Während die Vorhersage des Durbin-Levinson-Algorithmus stark von den realisierten Werten abweicht, entspricht die Vorhersage des Innovation-Algorithmus vor allem in den ersten Schritten sehr gut den realisierten Werten. 
...
```
Hier geht die Vorhersage mit Durbin-Levinson schnell gegen den Mittelwert, ohne die Schwankungen zu antizipieren. Im Gegensatz dazu erkennt der Innovation-Algorithmus das Periodische Verhalten der Temperaturschwankungen deutlich besser.
...
acf_plot_2
```
Man erkennt das starke Fallen der ACF mit dem lag, was typtisch für einen MA-Prozess ist. Wie bereits oben erklärt können erneut die Maxima nicht sehr gut aufgelöst werden. Es lohnt sich also den entsprechenden ACF-Plot anzuschauen um zu entscheiden welche Prediction-Funktion die Vorhersage besser schätzen kann.  
