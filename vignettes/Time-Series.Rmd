...
## Einleitung
In dieser Vignette werden die verschiedenen Funktionen des Packages *TimeSeries* ausführlich erklärt. Das Package enthält Implementationen einger wichtiger Algorithmen zur Analyse und Vorhersage von ARMA-Prozessen aus *Introduction to Time-Series Analysis and Forecasting* (Brockwell et. al, 2002):
...
Um die Funktionen nutzen zu können, muss zuerst das Paket *TimeSeries* geladen werden. Die anderen Paket werden hier zu Demonstrationszwecken geladen.
```{r setup}
library(TimeSeries)
library(ggplot2)
library(tibble)
...
1. Als erstes sehen wir hier einen ARMA(1,1)-Prozess mit einer kleinen und einer großen Standartabweichung. Je größer die Standartabweichung ist desto größer sind die Schwankungen um den Mittelwert. Dieser ist bei uns immer als 0 festgelegt. Im Prinzip wird also einfach die Skalierung der y-Achse geändert.
