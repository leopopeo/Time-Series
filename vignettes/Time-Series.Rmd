...
set.seed(2)
ar_obj <- arima.sim(n = 100,list(ar = c(0.2,0.6)))
ar_acf <- ACF(ar_obj, lag = 20)
ma_obj <- arima.sim( n = 100,list(ma = c(0.5,0.9)))
ma_acf <- ACF(ma_obj, lag = 20)
...
Der Durbin-Levinson Algorithmus kann für Vorhersagen einer Zeitreihe genutzt werden. Der Algorithmus benutzt dabei eine linear Kombination aller vorherigen Zeitreihen Punkte um einen neuen Wert vorherzusagen. Dies wird duch die folgende Gleichung nahc Brookwell et al. (2002) ausgedrückt:
...
Die Funktion *DLA* benutzt den Durbin-Levinson Algorithums und gibt die geschätzen $\phi's$ zurück. Die Funktion bestimmt standartmäßig die maximale Anzahl an Rekursionen, also n-1, dieser Wert kann jedoch individuel mit dem *len*-Parameter gesetzt werden. Die erhalten $\phi's$ können nun für eine Vorhersage benutzt werden. \n
Im folgenden Beispiel wird die Funktion auf AR-generierte Daten angewendet:
```{r}
set.seed(2)
ar_obj <- arima.sim(n = 100,list(ar = c(0.2,0.5)))
dl <- DLA(ar_obj)
```
...
DLA(ar_obj, len=5)
```
Der Output zeigt für die beiden angegebenen Rekursionen die geschätzten $\phi's$ an. Die ersten beiden Werte entsprechen hier nahezu den angegeben ar-Werten in *arima.sim*. Für die Vorhersage müssen nun die $\phi_{n1},...,\phi_{nn}$ mit der Zeitreihe (außgenommen dem letzten Wert, da die maximale Rekusionszahl n-1 ist) multipliziert und anschließend summiert werden.
```{r}
res <- sum(ar_obj*dl[-100])
res
```
...
## Periodogramm
Das Periodogramm einer Zeitreihe $\{x_1,...,x_n\}$ ist Funktion die auf Stichproben basiert einen Schätzung für die Spektrale Dichte zurückgibt. Die Definition ist dabei nach Brookwell et al. (2002):
$$
I_n(\lambda)=\frac{1}{n} \Bigg| \sum \limits_{t=1}^{n}x_{t}e^{-it\lambda} \Bigg| ^2
$$
Dabei steht der Koeffizient $x_t$ für eine Combination von Sinusen und Cosinusen:
$$
...
Die Folge ${a_k}$ ist die diskrete Fourier Transformationn der Zeitreihe und berechet sich nach: 
$$
...
Der folgende Code soll dies veranschaulichen.
