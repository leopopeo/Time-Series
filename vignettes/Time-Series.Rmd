...
1. Als erstes sehen wir einen ARMA(1,1)-Prozess mit einer kleinen und einer großen Standartabweichung. Je größer die Standartabweichung ist, desto größer sind die Schwankungen um den Mittelwert. Dieser ist bei uns immer mit 0 festgelegt. Anschaulich betrachtet,  wird also  die Skalierung der y-Achse geändert.
...
2. Hier sehen wir zuerst einen AR(1)-Prozess mit einem kleinen *Phi*-Wert und anschließend mit einem großen. Man sieht sehr schön, dass der *Phi*-Wert mit Glattheit der Zeitreihe zusammenhängt, je größer der Wert, desto glatter ist die Funktion. Das Gleiche würde  bei einem MA(1)-Prozess mit großen und kleinen *Theta*-Werten passieren.
```{r, fig.width = 7, fig.asp = .5}
set.seed(3)
...
predict <- DL_prediction(arma,len=9)
...
$$
(ebd. S.73).
Die Funktion *innovation(X, lag = n)* berechnet auf diese Weise $\mathbf\Theta_n$, welches wir dann für die Vorhersage mit *ts_predict* verwenden können. Mit dem Parameter *lag*  kann dabei das $n$ gewählt werden, bis zu welchem $\theta_{n,i}$ berechnet wird.
```{r}
...
$$
Die *perio* implementiert nun den Periodogramm-Algorithmus und gibt für die $\omega$ im oben definierten Intervall den Wert $I_n(\lambda)$ zurück.
