...
## Einleitung
In dieser Vignette werden die verschiedenen Funktionen des Packages *TimeSeries* ausführlich erklärt. Das Package enthält Implementationen einiger wichtiger Algorithmen zur Analyse und Vorhersage von ARMA-Prozessen aus *Introduction to Time-Series Analysis and Forecasting* (Brockwell et. al, 2002):
...
1. Als erstes sehen wir einen ARMA(1,1)-Prozess mit einer kleinen und einer großen Standartabweichung. Je größer die Standartabweichung ist, desto größer sind die Schwankungen des Rauschterms um den Mittelwert. Dieser ist bei uns immer mit 0 festgelegt.
Der Plot benutzt dabei die Funktion *plot_timeseries* und erzeugt einen einfachen Linienplot der Zeitreihe. 
...
2. Hier sehen wir zuerst einen AR(1)-Prozess mit einem kleinen *Phi*-Wert und anschließend mit einem großen. Man sieht hier, dass der *Phi*-Wert mit Glattheit der Zeitreihe zusammenhängt, je größer der Wert, desto glatter ist die Funktion. Das Gleiche passiert bei einem MA(1)-Prozess mit großen und kleinen *Theta*-Werten.
```{r, fig.width = 7, fig.asp = .5}
set.seed(3)
...
Die Autokovarianz ist ein Begriff aus der Signalverarbeitung und gibt die Kovarianz einer Funktion bzw. eines Signals mit sich selbst zu einem früheren Zeitpunkt an. Die Funktion gibt also an, wie viel Ähnlichkeit die um eine gewisse Zeitdifferenz verschobene Folge mit der ursprünglichen Folge besitzt. Die Autokovarianz-Funktion ist nach Brockwell et al. (2002) (S. 19) für eine Zeitdifferenz h wie folgt definiert:

$$\gamma_x(h):=COV(X_{t+h},X_t) $$
Da die obige Funktion jedoch nur funktioniert, wenn das Model der Zeitreihe bekannt ist arbeitet unsere Funktion ur mit den gemessenen Daten. Die Funktion *ACF* gibt also für eine Zeitreihe $x_{1},...,x_{n}$ die geschätzte Sichtprobenkovarianz für die Zeitdifferenz lag = $0$ bis lag = $n-1$ an. Zusätzlich zur Input Zeitreihe kann die lag-Variable individuell angegeben werden. Es kann also die Anzahl der zurückgegebenen Autokovarianzen gewählt werden. Dafür wird folgende Formel genutzt:
...
Die Funktion *DL_predict* benutzt nun bereits oben beschriebene Verfahren um Vorhersagen für eine Zeitreihe basierend auf dem Durbin-Levinson Algorithmus zu treffen. Der Eintrag $X_{n+1}$ der Zeitreihe $X_1, ... , X_{n}$ wird dabei jeweils durch:
$$
X_{n+1}=\phi_{n1}X_n+...+\phi_{nn}X_1. 
$$
bestimmt. Mit Hilfe der Funktion *plot_timeseries* können wir als zweites Argument die Vorhersage übergeben und bekommen anschließend einen Linienplot ausgegeben. Dies sind für einen AR(1)-Prozess folgendermaßen aus:
```{r,fig.width = 7, fig.asp = .5}
set.seed(13)
...
Wir können nun die Koeffizienten $\theta_{n,i}$ rekursiv berechnen. $\kappa(i,j)$ ist dabei die Kovarianz.
$$
\begin{aligned}
v_{0} &= \kappa(1,1), \\
...
$$
(ebd. S.73).
Die Funktion *innovation(X, lag = n)* berechnet auf diese Weise $\mathbf\Theta_n$, welches wir dann für die Vorhersage mit *ts_predict* verwenden können. Mit dem Parameter *lag* $<$ *length(X)*  kann dabei das $n$ gewählt werden, bis zu welchem $\theta_{n,i}$ berechnet wird.
```{r}
...
Obige Gleichung können wir nun auch verwenden, um eine $h$-Schritt-Prognose des Prozesses zu machen. Sei dazu $m$ die Länge der Zeitreihe. Dann berechnen wir mit $\hat X_{m+h}$ eine h-Schritt Prognose. Die Ausgabe von ts_predict ist per Default der Vektor $(\hat X_{m+1},..., \hat X_{m+h})$ der Länge $h$ mit den Prognosewerten. Wird *all = TRUE* gesetzt wir der gesamte Vektor $\mathbf{\hat X_{m+h}}$ ausgegeben. Die Vorhersage mit dem Innovation-Algorithmus eignet sich besonders gut für $MA(q)$-Prozesse mit kleinem $q$. Hier wird die *ts_predict*-Funktion auf einen $MA(2)$-Prozess angewendet:
...
Die Folge ${a_k}$ ist die diskrete Fouriertransformation der Zeitreihe und berechet sich nach: 
$$
