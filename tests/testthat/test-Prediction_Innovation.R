test_that("Eingabe testen", {
  ...
  expect_error(ts_predict(c(3,4,5,1), steps=1.1), "len muss NULL oder ein Integer Wert sein!")
  expect_error(ts_predict(c(3,4,5,1), steps=c(1,1)), "len muss NULL oder ein Integer Wert sein!")
  expect_error(ts_predict(c(3,4,5,1), steps=NA), "len muss >= 1 sein!")
