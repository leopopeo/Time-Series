test_that("Eingabe testen", {
  ...
  expect_error(DL_prediction(c(3,4,5,1), len=1.1), "len muss >= 2 sein!")
  expect_error(DL_prediction(c(3,4,5,1), len=c(1,1)), "len muss >= 2 sein!")
  expect_error(DL_prediction(c(3,4,5,1), len=NA), "len muss NULL oder ein Integer Wert sein!")
